...
public class PredictorScreen : MonoBehaviour {
    ...
    public void PredictPattern()
    {
        float avg1, avg2,sum1=0, sum2=0,diff,avgScore,dayScore,weekScore, alcoholScore,lapseScore=0, feedbackScore,totalScore;
        float pSum = 0,nSum=0,feedback;
        for(int i= dose.Length-1; i> dose.Length-8; i--)
        {
            sum1 += dose[i];
        }
        avg1 = sum1 / 7;
        for (int i = dose.Length - 8; i > dose.Length - 15; i--)
        {
            sum2 += dose[i];
        }
        avg2 = sum2 / 7;
        ...
        feedbackScore = (feedback + lapseScore) / 2;
